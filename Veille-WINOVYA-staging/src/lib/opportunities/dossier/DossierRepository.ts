...
export class DossierRepository {
  ...
  async getConsolidationInput(opportuniteId: string): Promise<DossierConsolidationInput> {
    ...
    // Sprint 11B : seules les alertes encore actives (is_active = true,
    // colonne additive Sprint 11B) alimentent la consolidation. Une
    // alerte retirée logiquement (retrait manuel) ne doit plus jamais
    // être comptée ni recalculée tant qu'elle n'est pas réintégrée — voir
    // docs/sprint-11b-alerte-retrait.md.
    ...
    return {
      ...
      // Sprint 11B : nombreSignaux ne retombe plus jamais sur la
      // colonne stockée opportunites.nombre_signaux (potentiellement
      // obsolète ou incluant des alertes depuis retirées) — toujours le
      // compte réel des signaux actifs rechargés ci-dessus.
      nombreSignaux: signaux.length,
