...
export function translateError(e: unknown, fallback: string = GENERIC_ERROR_MESSAGE): string {
  ...
  if (e instanceof OpportuniteIntrouvableError) {
    return 'Cette opportunité est introuvable (elle a peut-être été supprimée).'
  }
  if (e instanceof LienAlerteOpportuniteIntrouvableError) {
    return "Cette alerte n'est plus liée à cette opportunité."
  }
  if (e instanceof AlerteDejaRetireeError) {
    return 'Cette alerte a déjà été retirée de cette opportunité.'
  }
  if (e instanceof AlerteDejaActiveError) {
    return "Cette alerte est déjà active sur cette opportunité : rien à réintégrer."
  }
  if (e instanceof MotifRetraitInvalideError) {
    return 'Veuillez choisir un motif de retrait valide (un commentaire est requis pour "Autre").'
  }
  if (e instanceof RetraitNonAppliqueError) {
    return "Vous n'avez pas les droits nécessaires pour effectuer cette action, ou son état a changé entre-temps."
  }
