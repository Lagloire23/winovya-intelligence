...
import { Search, Loader2, Mail, Phone, Users, Building2, MapPin, Globe, TrendingUp, Share2 } from 'lucide-react'
import { supabase } from '../lib/supabase'
import type { Decideur } from '../lib/types'
import { LinkedinLink } from '../components/LinkedinLink'
import { DecideurEnrichButton } from '../components/DecideurEnrichButton'
...
function ReseauGraph({ nomEntreprise, liens }: { nomEntreprise: string; liens: EntrepriseLiee[] }) {
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-[460px] mx-auto" role="img" aria-label="Réseau d'entreprises liées">
        {shown.map((ent, i) => {
        ...
        {shown.map((ent, i) => {
          ...
          const x = center + radius * Math.cos(angle)
          const y = center + radius * Math.sin(angle)
          const labelY = y + (Math.sin(angle) >= 0 ? 1 : -1) * (nodeR + 14)
          return (
            <g key={ent.siren}>
              <circle cx={x} cy={y} r={nodeR} fill="hsl(217,42%,45%)" fillOpacity={0.12} stroke="hsl(217,42%,45%)" strokeWidth={1.5} />
              <text x={x} y={y + 4} textAnchor="middle" fontSize={11} fill="hsl(217,42%,45%)" fontWeight={700}>
                {truncate(ent.nom, 2).toUpperCase()}
              </text>
              <text x={x} y={labelY} textAnchor="middle" fontSize={10} fill="hsl(217,10%,25%)">
                {truncate(ent.nom, 20)}
              </text>
            </g>
          )
        ...
        <text x={center} y={center + 4} textAnchor="middle" fontSize={11} fill="hsl(149,100%,27%)" fontWeight={700}>
          {truncate(nomEntreprise, 16)}
        </text>
      </svg>
      <div className="flex items-center justify-center gap-4 mt-2 text-xs text-[hsl(217,4%,46%)]">
...
export function DecideursPage() {
  ...
  async function search() {
    ...
    // rôle de la page "Trouver le maire ou élu·e d'une commune"). On matche
    ...
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-heading font-bold text-brand-navy mb-1 flex items-center gap-2">
          <Users size={22} /> Décideurs &amp; organigrammes
        </h1>
        <p className="text-sm text-[hsl(217,4%,46%)]">
          Recherchez un décideur ou une entreprise dans notre base et, pour les entreprises privées, complétée automatiquement.
        </p>
      </div>
      ...
      <div className="card-winovya p-5 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          />
          <button onClick={search} disabled={!query.trim() || loading} className="btn-primary shrink-0 sm:w-auto">
      ...
      <div className="space-y-4">
        ...
        {!loading && pappersStatus === 'found' && company && (
          <div className="grid lg:grid-cols-3 gap-5">
            <div className="card-winovya p-5 lg:col-span-2">
              <p className="text-xs font-semibold text-brand-navy uppercase tracking-wide mb-3 flex items-center gap-1.5">
                <Building2 size={13} /> Informations
              </p>
              <p className="text-base font-semibold text-brand-navy">{company.nom}</p>
              {(company.formeJuridique || company.libelleNaf) && (
                <p className="text-sm text-[hsl(217,4%,46%)] mt-0.5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-2 text-xs mt-4">
                {company.siren && (
                  <div>
                    <span className="block text-[hsl(217,4%,46%)]">SIREN</span>
                    {company.siren}
                  </div>
                )}
                {company.siret && (
                  <div>
                    <span className="block text-[hsl(217,4%,46%)]">SIRET siège</span>
                    {company.siret}
                  </div>
                )}
                {company.dateCreation && (
                  <div>
                    <span className="block text-[hsl(217,4%,46%)]">Créée le</span>
                    {new Date(company.dateCreation).toLocaleDateString('fr-FR')}
                  </div>
                )}
                {company.effectif && (
                  <div>
                    <span className="block text-[hsl(217,4%,46%)]">Effectif</span>
                    {company.effectif}
                  </div>
                )}
              </div>
              <div className="flex flex-wrap gap-4 mt-4 text-sm border-t border-[hsl(217,6%,90%)] pt-3">
                {company.email && (
                  <a href={`mailto:${company.email}`} className="inline-flex items-center gap-1.5 text-brand-primary">
                    <Mail size={14} /> {company.email}
                  </a>
                )}
                {company.telephone && (
                  <a href={`tel:${company.telephone}`} className="inline-flex items-center gap-1.5 text-brand-primary">
                    <Phone size={14} /> {company.telephone}
                  </a>
                )}
                {company.siteWeb && (
                  <a
                  ...
                  >
                    <Globe size={14} /> {company.siteWeb}
                  </a>
                )}
              </div>
              {company.adresse && (
                <p className="text-sm text-[hsl(217,4%,46%)] mt-3 flex items-center gap-1.5">
                  <MapPin size={14} className="shrink-0" /> {company.adresse}
                </p>
              )}
              {company.latitude != null && company.longitude != null && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-sm text-brand-primary mt-1 inline-flex items-center gap-1 hover:underline ml-[20px]"
                >
                  Voir sur la carte
                </a>
              )}
                ...
                <dl className="text-sm space-y-2">
                  ...
                  {company.finances.margeBrute !== null && (
                    <div className="flex justify-between gap-2">
                      ...
                      <dd className="font-semibold text-brand-navy text-right">{formatMontant(company.finances.margeBrute)}</dd>
                    </div>
                  )}
                  ...
                  {company.finances.resultatNet !== null && (
                    <div className="flex justify-between gap-2">
                      ...
                      <dd className="font-semibold text-brand-navy text-right">{formatMontant(company.finances.resultatNet)}</dd>
                    </div>
                  )}
                </dl>
              </div>
